...
var formname = "wfPlanillaSocios.aspx";
var menuoption = "PlanillaSocios";
var asig = 0;
...
function CalculaTotales() {
    ...
    for (var r = 0; r < htmltable.rows.length; r++) {
        ...
        if (hijotot.length > 0) {
            ...
            desc = htmltable.rows[r].cells[c].innerText;
            valor = htmltable.rows[r].cells[c + 1].innerText;
            iva = (htmltable.rows[r].cells[c + 2].innerText == "SI") ? true : false;
        }
...
function MoveRigth() {
    ...
    for (var r = 1; r < rows.length; r++) {
        var obj = rows[r];
        $(obj).css("background-color", rgbcolor)
        $(obj).children("td").css("background-color", rgbcolor)
...
function ValidateForm() {
    ...
    if (count <1) {
        retorno = false;
        mensajehtml += "Es necesario selecionar  al menos una factura  para agregar a la planilla <br>";
    }
...
function GetTotalObj() {
    ...
    obj["tot_tseguro"] = ($.isNumeric($("#txtSEGURO").val())) ? parseFloat($("#txtTOTALSEG").val()) : null;
...
function GetDetalle() {
    ...
    for (var r = 1; r < rows.length; r++) {
        var obj = rows[r];
        if ($(obj).css("background-color") == rgbcolor) {
            detalle[r] = GetDcomdocObj(rows[r]);
        }
    }
...
function GetDcomdocObj(row) {
    var obj = {};
    obj["dca_comprobante"] = parseInt($(row).data("codpro").toString());
    obj["dca_comprobante_key"] = parseInt($(row).data("codpro").toString());
    obj["dca_empresa"] = parseInt(empresasigned["emp_codigo"]);
    obj["dca_empresa_key"] = parseInt(empresasigned["emp_codigo"]);
    obj["rfac_estado"] = parseInt(asig);
...
function GetComprobanteObj() {
    ...
    obj["com_codclipro"] = parseInt($("#txtCODCHOFER").val());
