//Archivo:          PagoSocio.js
//Descripción:      Contiene las funciones comunes para la interfaz de Pago Socios
...
var formname = "wfPagoSocio.aspx";
var menuoption = "PagoSocio";
...
function SaveObj() {
    if (ValidateForm()) {
        ...
        var jsonText = JSON.stringify({ objeto: obj });
        jConfirm('¿Está seguro que desea guardar el pago ? \n Luego no se podra modificar', 'Guardar', function (r) {
            if (r) {
...
function SaveObjResult(data) {
    if (data != "") {
        ...
        if (data.d != "-1") {
            ...
            jQuery.alerts.dialogClass = 'alert-success';
            jAlert('Pago socio guardado exitosamente...', 'Éxito', function () {
                jQuery.alerts.dialogClass = null; // reset to default
                //window.location = formname + "?codigocomp=" + data.d;
                $("#txtcodigocomp").val(data.d);
                PrintObj();
