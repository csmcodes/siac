...
var formname = "wfPlanillaClienteCOMY.aspx";
var menuoption = "PlanillaClienteCOMY";
...
$(document).ready(function () {
    ...
    //$("#txttipodoc").val(GetQueryStringParams("tipodoc"));
...
function GetCabeceraComprobanteResult(data) {
    ...
    $("#barracomp > ul").append("<li><div class=\"btn\" id=\"printdet\"><i class=\"iconfa-print\"></i> &nbsp; Imprimir Detalle</div></li>")
    $("#close").on("click", SaveObj);
    $("#save").on("click", SaveObj);
    $("#print").on("click", PrintObj);
    $("#printdet").on("click", PrintDetObj);
    $("#invo").on("click", SaveInvoiceObj);
...
function CalculaTotales() {
    ...
    for (var r = 0; r < htmltable.rows.length; r++) {
        ...
        sub0 = $(htmltable.rows[r].cells[c - 5]).text();
        sub = $(htmltable.rows[r].cells[c - 4]).text();
        iv = $(htmltable.rows[r].cells[c - 3]).text();
        seg = $(htmltable.rows[r].cells[c - 2]).text();
        tra = $(htmltable.rows[r].cells[c - 1]).text();
        tot = $(htmltable.rows[r].cells[c]).text();
        ...
        totalcal += ($.isNumeric(tot)) ? parseFloat(tot) : 0;
    ...
    iva = (subtotal) * (porcentajeiva / 100);//YA NO SE TOmA EN CUENTA EL SEGURO PARA EL IVA EN CARLOGISTICA
    ...
    //total = (subtot0 + transporte) + (subtotal + seguro) + iva;

    total = (subtot0 + transporte) + (subtotal) + iva;


    //CODIGO PARA SOLUCIONAR DESCUADRE
    var descuadre = totalcal - total;
    if (descuadre > 0) {
        if (seguro > 0)
            seguro += descuadre;
        else if (subtotal > 0)
    ...
    $("#txtSUBTOTAL0").val(CurrencyFormatted(subtot0 + transporte));
...
function SaveInvoiceObj() {

    

    if ($("#txtESTADO").val() == $("#txtCERRADO").val() && $("#txtFACTURA").val()!="") {
        InvoiceObj();
    }
    else {
        if (!saving) {
            if (ValidateForm()) {
                if (ValidaPlanilla()) {
                    ...
                    CallServer(formname + "/SaveObject", jsonText, 9);
                }
            }
        }
    }
...
function PrintObj() {
    ...
    var url = "./reports/wfReportPrint.aspx?report=PLACLI&empresa=" + parseInt(empresasigned["emp_codigo"]) + "&parameter1=" + $("#txtcodigocomp").val();
    var feautures = "top=0,left=0,width='+(screen.availWidth)+',height ='+(screen.availHeight)+',toolbar=0 ,location=0,directories=0,status=0,menubar=0,resizable=yes,scrollbars=yes";
...
function PrintDetObj() {
    ...
    var url = "./reports/wfReportPrint.aspx?report=PLACLIDET&empresa=" + parseInt(empresasigned["emp_codigo"]) + "&parameter1=" + $("#txtcodigocomp").val();
    var feautures = "top=0,left=0,width='+(screen.availWidth)+',height ='+(screen.availHeight)+',toolbar=0 ,location=0,directories=0,status=0,menubar=0,resizable=yes,scrollbars=yes";
    window.open(url, "Reporte", feautures);
...
function ValidaPlanilla() {
    ...
    var htmltable = $("#tddatos")[0];
    for (var r = 1; r < htmltable.rows.length; r++) {
        if ($(htmltable.rows[r].cells[2]).text().indexOf("FAC") >= 0) {
            fac = true;
            break;
        }
    }
    ...
    if (data != "") {
        if (data.d != "-1") {
            ...
            jAlert('Comprobante guardado exitosamente...', 'Éxito', function () {
                jQuery.alerts.dialogClass = null; // reset to default
                window.location.href = formname + "?codigocomp=" + data.d + "&tipodoc=" + GetQueryStringParams("tipodoc");
...
function SaveInvoiceResult(data) {
