...
{
    public class vGuiasDAL
    {
        #region Get All

        public static List<vGuias> GetAll(WhereParams parametros, string OrderBy)
        {
            List<vGuias> list = new List<vGuias>();
            vGuias obj = new vGuias();
        ...
        public static List<vGuias> GetAllByPlanilla(WhereParams parametros, string OrderBy)
        {
            ...
            if (DAL.GetProvider() == Provider.SqlServer)
            {
                ...
                {
                    transaction.BeginTransaction();
                    IDataReader reader = SqlDataBase.DB.GetAll(transaction, parametros, OrderBy, obj.GetSQLPlanilla());
                    do
                    {
                        if (!reader.Read())
                            break; // we are done
            ...
            else if (DAL.GetProvider() == Provider.PostgreSQL)
            {
                ...
                {
                    transaction.BeginTransaction();
                    IDataReader reader = SqlDataBasePG.DB.GetAll(transaction, parametros, OrderBy, obj.GetSQLPlanilla());
                    do
                    {
                        if (!reader.Read())
                            break; // we are done
