...
namespace WebUI
{
    public partial class wfComprobante : System.Web.UI.Page
    {
        ...
        public static string ShowObject(Comprobante obj)
        {
            ...
            List<Planillacli> lstplanilla = PlanillacliBLL.GetAll(new WhereParams("plc_empresa ={0} and plc_comprobante ={1}", obj.com_empresa, obj.com_codigo), "");
        ...
        public static string GetDetalle(object objeto)
        {
            ...
            List<Planillacli> lstplanilla = PlanillacliBLL.GetAll(new WhereParams("plc_empresa ={0} and plc_comprobante ={1}", comprobante.com_empresa, comprobante.com_codigo), "");
            if (lstplanilla.Count > 0)
                habilitado = false;
        ...
        public static string GetDetalleFromLGC(object objeto)
        {
            ...
            row.cells.Add(new HtmlCell { valor = Formatos.CurrencyFormat(lst.Sum(s => (s.detalle_subtot_0 ?? 0) + (s.detalle_subtotal ?? 0))), clase = Css.right });
            row.cells.Add(new HtmlCell { valor = Formatos.CurrencyFormat(0), clase = Css.right });
            //row.cells.Add(new HtmlCell { valor = Formatos.CurrencyFormat(comprobante.total.tot_subtot_0 + comprobante.total.tot_subtotal), clase = Css.right });
            row.cells.Add(new HtmlCell { valor = Formatos.CurrencyFormat(lst.Sum(s => (s.detalle_subtot_0 ?? 0) + (s.detalle_subtotal ?? 0))), clase = Css.right });
        ...
        [WebMethod]
        public static string GetPieFromLGC(object objeto)
        {
            Comprobante comprobante = new Comprobante(objeto);
            DateTime fecha = comprobante.com_fecha;
            ...
            comprobante.total = new Total();
            comprobante.total.tot_tseguro = lst.Sum(s => Math.Round((s.detalle_seguro??0),2));
            comprobante.total.tot_transporte = lst.Sum(s => Math.Round((s.detalle_transporte??0),2));
            comprobante.total.tot_timpuesto = lst.Sum(s => Math.Round((s.detalle_iva?? 0),2));
            comprobante.total.tot_desc1_0 = 0;
            comprobante.total.tot_desc2_0 = 0;
            comprobante.total.tot_descuento1 = 0;
            comprobante.total.tot_descuento2 = 0;
            comprobante.total.tot_subtot_0 += lst.Sum(s => Math.Round((s.detalle_subtot_0 ?? 0),2));
            comprobante.total.tot_subtotal += lst.Sum(s => Math.Round((s.detalle_subtotal ?? 0), 2));
            comprobante.total.tot_total = lst.Sum(s => Math.Round(s.detalle_total ?? 0, 2));


            //Verifica descuadre
            decimal total = comprobante.total.tot_subtot_0 + comprobante.total.tot_transporte + comprobante.total.tot_subtotal + (comprobante.total.tot_tseguro ?? 0) + comprobante.total.tot_timpuesto;

            if (comprobante.total.tot_total != total)
            {
                decimal descuadre = comprobante.total.tot_total - total;

                if (descuadre > 0 && descuadre<1)
                    comprobante.total.tot_tseguro += descuadre;
            ...
            tdatos.rows[1].cells[0].valor = "Subtotal:";
            tdatos.rows[1].cells[1].valor = new Input { id = "txtSUBTOTAL0", clase = Css.small + Css.amount, habilitado = false, valor = Formatos.CurrencyFormat(comprobante.total.tot_subtot_0 + comprobante.total.tot_transporte ) }.ToString();
            tdatos.rows[1].cells[2].valor = new Input { id = "txtSUBTOTALIVA", clase = Css.small + Css.amount, habilitado = false, valor = Formatos.CurrencyFormat(comprobante.total.tot_subtotal + comprobante.total.tot_tseguro) }.ToString();
            tdatos.rows[2].cells[0].valor = "Desc:";
            tdatos.rows[2].cells[1].valor = new Input { id = "txtDESC0", clase = Css.small + Css.amount, habilitado = false, valor = Formatos.CurrencyFormat(comprobante.total.tot_desc1_0) }.ToString();
            tdatos.rows[2].cells[2].valor = new Input { id = "txtDESCIVA", clase = Css.small + Css.amount, habilitado = false, valor = Formatos.CurrencyFormat(comprobante.total.tot_descuento1) }.ToString();
            tdatos.rows[3].cells[0].valor = "Descuento:";
        ...
        public static string SaveObject(object objeto)
        {
            ...
            try
            {
                ...
                else if(obj.com_tipodoc == Constantes.cGuia.tpd_codigo)
                    obj = FAC.close_guia(obj);
                ...
                //NUEVO PORTAL DE CONSUTLA, ENVIA COMPROBANTE
                External.GenerarComprobanteAsync(obj);
        ...
        [WebMethod]
        public static string GetAgente(object id)
        {
