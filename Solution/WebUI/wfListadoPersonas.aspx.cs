...
namespace WebUI
{
    public partial class wfListadoPersonas : System.Web.UI.Page
    {
        protected static int pageIndex;
        protected static int pageSize;
        protected static string OrderByClause = "per_nombres, per_apellidos limit 50";
        ...
        public static string GetFiltros(object objeto)
        {
            ...
            tdatos.rows[0].cells[1].valor = new Input { id = "txtIDB", clase = Css.large, }.ToString();
            tdatos.rows[1].cells[0].valor = "Nombre";
            tdatos.rows[1].cells[1].valor = new Input { id = "txtNombre", clase = Css.large }.ToString();
        ...
        [WebMethod]
        public static string LoadDetalle()
        {
            return null;
          ...
          [WebMethod]
        public static string LoadData(object objeto)
        {
            ...
            tdatos.invoice = true;
            tdatos.AddColumn("", "width5", Css.center, new Boton { editrow = true, tooltip = "Editar registro" }.ToString());
            tdatos.AddColumn("", "width5", Css.center, new Boton { removerow = true, tooltip = "Eliminar registro" }.ToString());
            tdatos.AddColumn("Id", "width5", "", "");
            tdatos.AddColumn("Ruc", "width10", "", "");
            tdatos.AddColumn("Nombres", "width20", "", "");
            tdatos.AddColumn("Razon", "width20", "");
            tdatos.AddColumn("Activo ", "width2", new Check() { id = "chkActivo", clase = Css.blocklevel + Css.cantidades, habilitado = false, valor = 0 }.ToString());
            tdatos.editable =false;

            if (nombre != "")
            {
                parametros.where += ((parametros.where != "") ? " and " : "") + " per_nombres LIKE  upper({" + contador + "}) ";
                valores.Add("%" +nombre+ "%");
                contador++;
                parametros.where += ((parametros.where != "") ? " or " : "") + " per_apellidos LIKE upper(  {" + contador + "} )";
                valores.Add("%" + nombre + "%");
                contador++;
                parametros.where += ((parametros.where != "") ? " or " : "") + " per_razon LIKE upper( {" + contador + "} )";
                valores.Add("%" + nombre + "%");
                contador++;
            }
            if (id != "")
            {
                parametros.where += ((parametros.where != "") ? " and " : "") + " per_id = {" + contador + "} ";
                valores.Add(""+id+"");
                contador++;
                parametros.where += ((parametros.where != "") ? " or " : "") + " per_ciruc = {" + contador + "} ";
                valores.Add(""+id+"");
                contador++;
            }
           ...
           // List<Persona> personas = vPersonaBLL.GetAllByPage(parametros, OrderByClause,desde,hasta);

           List<Persona> personas = vPersonaBLL.GetAll(parametros, OrderByClause);
            foreach (Persona item in personas)
            {

                HtmlRow row = new HtmlRow();
                row.removable = false;
                row.data = "data-persona=" + item.per_codigo+" data-empresa= "+item.per_empresa;
               // row.markable = true;
                row.cells.Add(new HtmlCell { valor = new Boton { editrow = true, tooltip = "Editar registro" }.ToString(), clase = Css.center });
                ...
                row.cells.Add(new HtmlCell { valor = item.per_id });
                row.cells.Add(new HtmlCell { valor = item.per_ciruc });  
                row.cells.Add(new HtmlCell { valor = item.per_nombres +" "+ item.per_apellidos});
                row.cells.Add(new HtmlCell { valor = item.per_razon });

                row.cells.Add(new HtmlCell { valor = new Check() { id = "chkActivo", habilitado = false, valor = ((item.per_estado.HasValue) ? (item.per_estado.Value == 1) ? 1 : 0 :0 ) }.ToString() });
                //row.cells.Add(new HtmlCell { valor = item.per_estado });
               // row.cells.Add(new HtmlCell { valor =((item.per_estado.HasValue)? (item.per_estado.Value==1)?"SI":" NO ":"NO") , clase = Css.center});
          ...
          [WebMethod]
          public static string Delete(object objeto)
          {
