...
var formname = "wfComprobante.aspx";
var menuoption = "Comprobante";
...
function LoadPie() {
    ...
    else if (origen == "LGC")  //PLANILLA DE CLIENTES
    {
        obj["com_codigo"] = $("#txtcodigocompref").val();
        var jsonText = JSON.stringify({ objeto: obj });
        CallServer(formname + "/GetPieFromLGC", jsonText, 2);
    }
...
function GetAutoCompleteObj(idobj, item) {
    ...
    if (idobj == "txtCIRUC_P") {
        ...
        $("#txtNOMBRES_P").prop("disabled", true);
        $("#txtAPELLIDOS_P").prop("disabled", true);
        $("#txtRAZON_P").prop("disabled", true);
        $("#txtDIRECCION_P").prop("disabled", true);
        $("#txtTELEFONO_P").prop("disabled", true);
        $("#txtMAIL_P").prop("disabled", true);
        $("#cmbTIPOID_P").prop("disabled", true);
...
function CalculaTotales() {
    ...
    var seguro = parseFloat((valordeclarado * (porcentajeseguro / 100)).toFixed(2));
...
function Edit(row) {
    ...
    $("#txtTOTAL").val($(row.cells[8]).text()); $(row.cells[8]).text("");
...
function GetComprobanteObj() {
    ...
    obj["com_fecha"] = new Date(currentDate.getFullYear(), currentDate.getMonth(), currentDate.getDate(), now.getHours(), now.getMinutes(), now.getSeconds(), now.getMilliseconds());
    obj["com_fecha_manual"] = $("#txtFECHACOMP_MANUAL").val() === "true"; //la fecha final la resuelve el servidor (ver General.ResolveFechaComprobante), esto solo indica si el dia fue elegido a proposito
...
function ValidateForm() {
    ...
    $($("#txtCODCLIPRO")[0].parentNode).removeClass('obligatorio')
    $($("#txtCODCLIPRO")[0].parentNode).children(".obligatorio").remove();

    var codigoper = parseInt($("#txtCODPER").val());
    if (codigoper==0)
    {
        retorno = false;
        var padre = $($("#txtCODCLIPRO")[0].parentNode);
        padre.append("<span class='obligatorio'>! Obligatorio</span>");
        padre.addClass('obligatorio')
        mensajehtml += "La seleccion de  un cliente  <b>" + $("#txtCODCLIPRO").attr('placeholder') + "</b> es obligatorio <br>";
    }
...
$(window).keydown(function (event) {
    ...
    if (id == "txtIDPRO" || id == "txtOBSERVACION" || id == "txtPRECIO" || id == "txtCANTIDAD" || id == "txtDESC" || id == "txtPESO" || $("#" + id).hasClass('calculo')) {
