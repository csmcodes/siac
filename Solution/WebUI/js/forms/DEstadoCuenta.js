...
var rgbcolor = "rgb(176, 196, 222)"
 var formname = "wfDEstadoCuenta.aspx";
var menuoption = "DEstadoCuenta";
...
function GetCabeceraComprobante(per_codigo, ddo_debcre, codalmacen, fechacort) {
    if (ddo_debcre >= 0) {
...
function LoadCabeceraResult(data) {
    ...
    if (data != "") {
        ...
        GetCabeceraComprobante($("#txtcodpersona").val(), $("#txtdebcre").val(), $("#txtcodigoalm").val(), $("#txtfechacorte").val());
    }
    $('.fecha').datepicker({ dateFormat: 'dd/mm/yy' }).val();
}
function LoadDetalle() {
    var obj = {};
    obj["per_codigo"] = parseInt($("#cmbNOMBRE").val());
    obj["com_almacen"] =parseInt( $("#cmbALMACEN").val());
    obj["ddo_fecha_emi"] = $("#cmbCORTE").val();
    obj["ddo_debcre"] = $("#txtdebcre").val();   
...
function PrintEstado() {
    ...
    window.location = "wfestadocuentaprint.aspx?debcre=" + $("#txtdebcre").val() + "&codpersona=" + parseInt($("#cmbNOMBRE").val()) + "&codalmacen=" + codalmacen + "&fechacort=" + $("#cmbCORTE").val();
}

function PrintCartera() {
    //   window.location = "wfComprobantePrint.aspx?codigocomp=" + $("#txtcodigocomp").val();
    var codalmacen = parseInt($("#cmbALMACEN").val()) || 0;
    window.location = "wfcarteraprint.aspx?debcre=" + $("#txtdebcre").val() + "&codpersona=" + parseInt($("#cmbNOMBRE").val()) + "&codalmacen=" + codalmacen + "&fechacort=" + $("#cmbCORTE").val();
}
