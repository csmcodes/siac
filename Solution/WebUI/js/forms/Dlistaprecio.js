//Archivo:          AsignarCaracteristicas.js
//Descripción:      Contiene las funciones propias de la interfaz
//Desarrollador:    Cristhian Sanmartin M.
//Fecha:            Julio 2013
//2013. Gestión Tecnológica GTEC Cía. Ltda. Todos los derechos reservados

var selectobj = null; //contiene el objeto seleccionado en el listado
var color = "LightSteelBlue";
var rgbcolor = "rgb(176, 196, 222)"
var formname = "wfDlistaPrecio.aspx";
var menuoption = "DlistaPrecio";
var precio = null;
var estado = null;
//Codigo ejecutado cuando el document esta listo
$(document).ready(function () {
    $("#masterdetailcontent").on("scroll", scroll);
    LoadCabecera();
    //LoadDetalle();
    LoadForm();     //Carga el formulario
    $("#saveedit").on("click", SaveObj);    //Opción "Guardar" del combo de opciones de la sección de edición
    $("#addnew").on("click", AddObj);    //Opción "Guardar" del combo de opciones de la sección de edición
    $("#remove").on("click", DeleteObjs);    //Opción "Eliminar" del combo de opciones de la sección de edición
    $('body').css('background', 'transparent'); //Limpia el fondo de la pantalla
});
function StopPropagation(event) {
    if (!event) var event = window.event;
    event.cancelBubble = true;
    if (event.stopPropagation) event.stopPropagation();
}

//Funciona que invoca al servidor mediante JSON
function CallServer(strurl, strdata, retorno) {
    $.ajax({
        type: "POST",
        url: strurl,
        data: strdata,
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: function (data) {
            if (retorno == 0)
                LoadCabeceraResult(data);
            if (retorno == 1)
                LoadDetalleResult(data);
            if (retorno == 2)
                LoadFormResult(data);
            if (retorno == 3)
                ShowObjResult(data);
            if (retorno == 4)
                SaveObjResult(data);
            if (retorno == 5)
                DeleteObjsResult(data);
            if (retorno == 6)
                GetObjectRowResult(data);
        },
        error: function (XMLHttpRequest, textStatus, errorThrown) {
            var errorData = $.parseJSON(XMLHttpRequest.responseText);
            jQuery.alerts.dialogClass = 'alert-danger';
            jAlert(errorData.Message, 'Error', function () {
                jQuery.alerts.dialogClass = null; // reset to default
            });
        }
    })
}

function scroll() {

    if ($("#masterdetailcontent")[0].scrollHeight - $("#masterdetailcontent").scrollTop() <= $("#masterdetailcontent").outerHeight()) {
            LoadDetalle();
      
    }
}
function ReloadData() {
    $('#detallebody').empty();
    var obj = {};
    obj["dlpr_nombreproducto"] = $("#cmbPRODUCTO_S").val();
    obj["dlpr_listaprecio"] = parseInt($("#cmbLISTAPRECIO_S").val());
    obj["dlpr_ruta"] = parseInt($("#cmbRUTA_S").val());
    obj["dlpr_almacen"] = parseInt($("#cmbALMACEN_S").val());
    obj["dlpr_umedida"] = parseInt($("#cmbUNIDAD_S").val());
    obj["dlpr_empresa"] = parseInt(empresasigned["emp_codigo"]);
    var jsonText = JSON.stringify({ id: obj });
  
    CallServer(formname + "/ReloadData", jsonText,1);
}

function LoadCabecera() {

    CallServer(formname + "/GetCabecera", "{}", 0);


}
function LoadCabeceraResult(data) {
    if (data != "") {
        $('#cabecera').append(data.d);
        $("#cmbPRODUCTO_S").keyup("change", ReloadData);
        $("#cmbLISTAPRECIO_S").on("change", ReloadData);
        $("#cmbRUTA_S").on("change", ReloadData);
        $("#cmbALMACEN_S").on("change", ReloadData);
        $("#cmbUNIDAD_S").on("change", ReloadData);
       
  
        
        LoadDetalle();
    }
    $('.fecha').datepicker({ dateFormat: 'dd/mm/yy' }).val();

}
function LoadDetalle() {
    var obj = {};
    obj["dlpr_nombreproducto"] = $("#cmbPRODUCTO_S").val(); 
    obj["dlpr_listaprecio"] = parseInt($("#cmbLISTAPRECIO_S").val());
    obj["dlpr_ruta"] = parseInt($("#cmbRUTA_S").val());
    obj["dlpr_almacen"] = parseInt($("#cmbALMACEN_S").val());
    obj["dlpr_umedida"] = parseInt($("#cmbUNIDAD_S").val());
    obj["dlpr_empresa"] = parseInt(empresasigned["emp_codigo"]); 
    var jsonText = JSON.stringify({ id: obj });
    CallServer(formname + "/GetDetalle", jsonText, 1);
}
function LoadDetalleResult(data) {

    if (data != "") {
       $("#detallebody tr:last").remove();
        $('#detallebody').append(data.d);
    }
    $('#cargando').empty();
    var htmltable = $("#detallebody")[0];
    for (var r = 0; r < htmltable.rows.length; r++) {
        var c = htmltable.rows[r];
        $(c).unbind('change');      
        $(c).on("change", GetObjectPrecio);
    
    }



   
}
function LoadForm() {
    CallServer(formname + "/GetForm", "{}", 2);
}
function LoadFormResult(data) {
    if (data != "") {
        $('#formmodal').html(data.d);
    }
    $(".fecha").datepicker({

        dateFormat: "dd/mm/yy"
    }); //Setea campos de tipo fecha
}
function StopPropagation(event) {
    if (!event) var event = window.event;
    event.cancelBubble = true;
    if (event.stopPropagation) event.stopPropagation();
}
/****************************FUNCIONES BASICAS*******************************/
function GetCheckValue(obj) {
    if ($(obj).length > 0) {
        if ($(obj)[0].checked)
            return 1;
        else
            return 0;
    }
    //return null;
}
function SetCheckValue(value) {
    if (value != null) {
        if (value.toString() == "1")
            return true;
    }
    return false;
    //return null;
}
function GetDateValue(value) {
    if (value != null) {
        var date = new Date(parseInt(value.substr(6)));
        return date.toLocaleString();
        //return eval(value.slice(1, -1));
    }
    return "";
}
function Edit(obj) {
    //var id = $(obj).data("id");
    selectobj = obj;
    
    //  ShowObj(obj);
 
}
function Select(obj) {
    //alert($(obj).css("background-color"));
    if ($(obj).css("background-color") == rgbcolor)
        $(obj).css("background-color", "");
    else
        $(obj).css("background-color", color);
}
function CleanSelect() {
    var rows = $("#detallebody tr");
    $(rows).css("background-color", "");
}
function ShowObj(obj) {
    var id = $(obj).data("id");
    var jsonText = JSON.stringify({ id: id });
    CallServer(formname + "/GetObject", jsonText, 3)
}
function DisableControls(disable) {
    $("#cmbLISTAPRECIO").attr("disabled", disable);
   
}
function ShowObjResult(data) {
    if (data != "") {
        var obj = $.parseJSON(data.d);
        SetJSONObject(obj); //TOCA DETERMINAR DEPENDIENDO DE CADA FORM
        DisableControls(true);
        $("#edit").trigger('click'); //MUESTRA EL DIV CON EL FORM
        CleanSelect();
    }
}

function GetJSONObject() {
    var obj = {};
    obj["dlpr_codigo"] = $("#txtCODIGO").val();
    obj["dlpr_codigo_key"] = $("#txtCODIGO_key").val();
    obj["dlpr_empresa"] =parseInt(empresasigned["emp_codigo"]);
    obj["dlpr_empresa_key"] =parseInt(empresasigned["emp_codigo"]);
    obj["dlpr_listaprecio"] = parseInt($("#cmbLISTAPRECIO_S").val());
    obj["dlpr_listaprecio_key"] = parseInt($("#cmbLISTAPRECIO_S").val());
    obj["dlpr_almacen"] = $("#cmbALMACEN").val();
    obj["dlpr_producto"] = $("#cmbPRODUCTO").val();
    obj["dlpr_umedida"] = $("#cmbUMEDIDA").val();
    obj["dlpr_fecha_ini"] = $("#cmbFECHA_INI").val();
    obj["dlpr_fecha_fin"] = $("#cmbFECHA_FIN").val();
    obj["dlpr_precio"] = $("#txtPRECIO").val();
    obj["dlpr_estado"] = GetCheckValue($("#chkESTADO"));
    obj["dlpr_idalmacen"] = $("#cmbALMACEN option:selected").text();
    obj["dlpr_nombreproducto"] = $("#cmbPRODUCTO option:selected").text();
    obj["dlpr_nombreumedida"] = $("#cmbUMEDIDA option:selected").text();

    obj["dlpr_ruta"] = $("#cmbRUTA").val();
    obj = SetAuditoria(obj);
    var jsonText = JSON.stringify({ objeto: obj });
    return jsonText;
}

function SetJSONObject(obj) {

    $("#txtCODIGO").val(obj["dlpr_codigo"]);
    $("#txtCODIGO_key").val(obj["dlpr_codigo"]);
    $("#cmbRUTA").val( obj["dlpr_ruta"]);
    $("#cmbLISTAPRECIO_S").val(obj["dlpr_listaprecio"]);   
    $("#cmbALMACEN").val(obj["dlpr_almacen"] );
    $("#cmbPRODUCTO").val(obj["dlpr_producto"]);
    $("#cmbUMEDIDA").val(obj["dlpr_umedida"] );
    $("#cmbFECHA_INI").val(GetDateValue(obj["dlpr_fecha_ini"]));
    $("#cmbFECHA_FIN").val(GetDateValue(obj["dlpr_fecha_fin"]) );
    $("#txtPRECIO").val(obj["dlpr_precio"]);
    $("#chkESTADO").prop("checked", SetCheckValue(obj["dlpr_estado"]));
    $("#lblcrea").html("Creado por: " + obj["crea_usr"] + " " + GetDateValue(obj["crea_fecha"]));
    $("#lblmod").html("Modificado por: " + obj["mod_usr"] + " " + GetDateValue(obj["mod_fecha"]));
}
function GetObjectPrecio() {

    //   row = $(this).parent().children().index($(this));  
 //   var verificaHorario = $(this).find("td#" + "chkESTADO")
   
//    $(verificaHorario[0]).data("id");
 precio = $(this).find('input[id=txtPRECIO]').val();
  estado = $(this).find('input[id=chkESTADO]');
   
  var id = $(this).data("id");
    var jsonText = JSON.stringify({ id: id });
    CallServer(formname + "/GetObject", jsonText, 6)
    StopPropagation();
}

function GetObjectEstado() {
    var r = $("#chkESTADO")[0].parentNode.parentNode;
    var id = $(r).data("id");
    var jsonText = JSON.stringify({ id: id });
    CallServer(formname + "/GetObject", jsonText, 6)

}
function GetObjectRowResult(data) {
 
     if (data != "") {
        var obj = $.parseJSON(data.d);
        obj["dlpr_precio"] = precio;
        obj["dlpr_estado"] = GetCheckValue(estado); 
        obj["dlpr_fecha_ini"]= GetDateValue(obj["dlpr_fecha_ini"]);
        obj["dlpr_fecha_fin"]= GetDateValue(obj["dlpr_fecha_fin"]);
        var jsonText = JSON.stringify({ objeto: obj });
        CallServer(formname + "/SaveObject", jsonText, 4);
    }
}




function RemoveRow(btn) {
    var row = $(btn).parents('tr');
    var hascontrols = ($(row).find("input").length > 0) ? true : false;
    var fr = $(row[0]).data("id");
    var objetos = []; 
    objetos.push($(row[0]).data("id"));
    fr.dlpr_codigo = parseInt(fr.dlpr_codigo);
    
    
    if (fr.dlpr_codigo > 0 ) {
        jConfirm('¿Está seguro que desea eliminar el registro?', 'Eliminar', function (r) {
            if (r) {
                $(btn).parents('tr').fadeOut(function () {
                    $(this).remove();
                    var jsonText = JSON.stringify({ objetos: objetos });
                    CallServer(formname + "/DeleteObjects", jsonText, 5)
                });
            }
        });
    }
    else
    CleanForm();   
    StopPropagation();
}

function CleanForm() {
    var dt = new Date();
    $("#txtCODIGO").val("");
    $("#txtCODIGO_key").val("");
    $("#cmbRUTA").val("");
    $("#cmbLISTAPRECIO_S").val("");
    $("#cmbALMACEN").val("");
    $("#cmbPRODUCTO").val("");
    $("#cmbUMEDIDA").val("");
    $("#cmbFECHA_INI").val("");
    $("#cmbFECHA_FIN").val("");
    $("#txtPRECIO").val("");
    $("#chkESTADO").prop("checked", true);
}
function ClearValidate() {
    var controles = $("#datoscontent").find('[data-obligatorio="True"]');
    $("#datoscontent").find('[data-obligatorio="True"]').each(function () {
        $(this.parentNode).removeClass('obligatorio')
        $(this.parentNode).children(".obligatorio").remove();
    });
}
function ValidateForm() {
    var retorno = true;
    /* var controles = $("#datoscontent").find('[data-obligatorio="True"]');
    $("#datoscontent").find('[data-obligatorio="True"]').each(function () {
    $(this.parentNode).removeClass('obligatorio')
    $(this.parentNode).children(".obligatorio").remove();
    if ($(this).val() == "") {
    retorno = false;
    var padre = $(this.parentNode);
    $(this.parentNode).append("<span class='obligatorio'>! Obligatorio</span>");
    $(this.parentNode).addClass('obligatorio')
    }

    });*/
    return retorno;
}
function SaveObj() {
    if (ValidateForm()) {
        var jsonText = GetJSONObject(); //TOCA DETERMINAR DEPENDIENDO DE CADA FORM   
        CallServer(formname + "/SaveObject", jsonText, 4);
    }

}
function SaveObjResult(data) {
    if (data != "") {
        if (data.d != "OK" && data.d.toString() != "ERROR") {
            if (selectobj != null) {
                jQuery.jGrowl("Registro actualizado con éxito");
           //     $(selectobj).replaceWith(data.d);
            }
            else {
                jQuery.jGrowl("Registro ingresado con éxito");
                $('#detallebody').append(data.d);
            }
            LoadDetalle();
        }
        /*else {
        CleanForm(); //TOCA DETERMINAR DEPENDIENDO DE CADA FORM  
        ReloadData();
        }*/
    }
}
function AddObj() {
    ClearValidate();
    DisableControls(false);
    $("#edit").trigger('click');
    selectobj = null;
    CleanForm(); //TOCA DETERMINAR DEPENDIENDO DE CADA FORM   
}
function DeleteObjs() {
    jConfirm('¿Está seguro que desea eliminar los registros seleccionados?', 'Eliminar', function (r) {
        if (r) {
            var objetos = [];
            $("#detallebody").find('tr').each(function () {
                if ($(this).css("background-color") != "transparent") {
                    objetos.push($(this).data("id"));
                }
            });
            var jsonText = JSON.stringify({ objetos: objetos });
            CallServer(formname + "/DeleteObjects", jsonText, 5)
        }
    });
}
function DeleteObjsResult(data) {
    if (data != "") {
        if (data.d == "OK") {
            $("#detallebody").find('tr').each(function () {
                if ($(this).css("background-color") != "transparent") {
                    $(this).remove();
                }
            });
            LoadDetalle();
        }
    }
}