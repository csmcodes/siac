...
var formname = "wfComprobanteTC.aspx";
var menuoption = "ComprobanteTC";
...
function GetPrintFormat() {
    ...
    var jsonText = JSON.stringify({ objeto: obj });
    CallServer("ws/Metodos.asmx/GetPrintFormat1", jsonText, 15);
}
...
function LoadDetalle() {
    ...
    if (origen == "") {
        ...
        obj["com_codigo"] = $("#txtcodigocompref").val();
        var jsonText = JSON.stringify({ objeto: obj });
        CallServer(formname + "/GetDetalleFromLGC1", jsonText, 1);
    }
...
function SetAutoCompleteObj(idobj, item) {
    if (idobj == "txtCODCLIPRO" || idobj == "txtIDAGE") {
        return {
...
function GetAutoCompleteObj(idobj, item) {
    ...
    if (idobj == "txtIDAGE") {
        $("#txtIDAGE").val(item.info.per_id);        
        $("#txtCODAGE").val(item.info.per_codigo);        
        $("#txtNOMBRESAGE").val(item.info.per_apellidos + " " + item.info.per_nombres);
...
function GetPriceResult(data) {
    ...
    if (data != "") {
        if (data.d != "-1") {
            $("#txtPRECIO").val(CurrencyFormattedAll(data.d));
        }
        else {
            jQuery.alerts.dialogClass = 'alert-danger';
...
function CalculaPrecioT() {
    ...
    $("#txtPRECIO").val((precio.toPrecision(8)));
    CalculaTotalesModificar();
}
...
function CalculaTotales() {
    ...
    for (var r = 0; r < htmltable.rows.length; r++) {
        ...
        var subtotal = (($.isNumeric(cant)) ? parseFloat(cant) : 0) * (($.isNumeric(prec)) ? parseFloat(prec) : 0);
        //var subtotaldesc = subtotal * (parseFloat(desc) / 100);
        var subtotaldesc = ($.isNumeric(desc)) ? parseFloat(desc) : 0;

        subtotalIVA += (iva) ? subtotal : 0;
        subtotal0 += (!iva) ? subtotal : 0;
    ...
    var valorIVA = (subtotalIVA - descIVA - descuentoiva + seguro) * (porcentajeiva / 100);

    subtotal0 += transporte;
    subtotalIVA += seguro;
...
//modifica los totales 


function CalculaTotalesModificar() {
    var htmltable = $("#tdinvoice")[0];
    var subtotal0 = 0;
    var subtotalIVA = 0;
    var desc0 = 0;
    ...
    for (var r = 0; r < htmltable.rows.length; r++) {
        ...
        var subtotal = (($.isNumeric(valor)) ? parseFloat(valor) : 0) + (($.isNumeric(desc)) ? parseFloat(desc) : 0) ;
...
//fin de modificacion de totales
...
function AddEditRow() {
    ...
    var newrow = $("<tr data-codpro='" + $("#txtCODPRO").val() + "' data-calcpro='" + $("#txtCALCPRO").val() + "' onclick='Edit(this);'></tr>");
...
function Edit(row) {
    ...
    $("#cmbUMEDIDA option[value='" + $(row.cells[4]).data("coduni") + "']").attr('selected', 'selected'); row.cells[4].innerText = "";
    $("#txtCANTIDAD").val(row.cells[5].innerText); row.cells[5].innerText = "";
    $("#txtPRECIO").val(row.cells[6].innerText); row.cells[6].innerText = "";
    $("#txtDESC").val(row.cells[7].innerText); row.cells[7].innerText = "";
    $("#txtTOTAL").val(row.cells[8].innerText); row.cells[8].innerText = "";
    $("#chkIVA").prop("checked", ((row.cells[9].innerText == "SI") ? true : false)); row.cells[9].innerText = "";
    $("#txtIDPRO").focus();
    ...
    r.cells[3].innerHTML = caract;

    $(r.cells[4]).data("coduni", coduni);
    r.cells[4].innerText = unidad;
    r.cells[5].innerText = cantidad;
    r.cells[6].innerText = precio;
    r.cells[7].innerText = desc;
    r.cells[8].innerText = total;
    r.cells[9].innerText = iva;
...
function EditCan(row) {
    ...
    $("#txtIDTIPO_P").val(row.cells[0].innerText); row.cells[0].innerText = "";
    $("#txtNOMBRETIPO_P").val(row.cells[1].innerText); row.cells[1].innerText = "";
    $("#txtVALOR_P").val(row.cells[2].innerText); row.cells[2].innerText = "";
    $("#txtIDTIPO_P").focus();
    ...
    r.cells[0].innerText = id;
    r.cells[1].innerText = nombre;
    r.cells[2].innerText = valor;
...
function GetDcomdocObj(row) {
    ...
    if (hijocodpro.length > 0) {
        if ($.isNumeric($("#txtCODPRO").val())) {
            ...
            obj["ddoc_producto"] = $("#txtCODPRO").val();
            obj["ddoc_observaciones"] = $("#txtOBSERVACION").val();
            obj["ddoc_peso"] = $("#txtPESO").val();
            obj["ddco_udigitada"] = $("#cmbUMEDIDA").val();
        ...
        if ($.isNumeric($(row).data("codpro").toString())) {
            ...
            obj["ddoc_producto"] = $(row).data("codpro").toString();
            obj["ddoc_observaciones"] = row.cells[2].innerText;
            obj["ddoc_peso"] = row.cells[3].innerText;
            obj["ddco_udigitada"] = $(row.cells[4]).data("coduni");
            obj["ddoc_cantidad"] = parseFloat(row.cells[5].innerText);
            obj["ddoc_precio"] = parseFloat(row.cells[6].innerText);
            obj["ddoc_dscitem"] = parseFloat(row.cells[7].innerText);
            obj["ddoc_total"] = parseFloat(row.cells[8].innerText);
            obj["ddoc_grabaiva"] = ((row.cells[9].innerText == "SI") ? 1 : 0);
        }
        else
            return null;
    }
...
function GetCcomenvObj() {
    ...
    if ($.isNumeric($("#txtGUIAS").val()))
        obj["cenv_guias"] = $("#txtGUIAS").val();
    obj = SetAuditoria(obj);
    return obj;
}
...
function GetComprobanteObj() {
    ...
    obj["com_agente"] = parseInt($("#txtCODAGE").val());
...
$(window).keydown(function (event) {
    ...
    if (id == "txtIDPRO" || id == "txtOBSERVACION" || id == "txtPRECIO" || id == "txtCANTIDAD" || id == "txtDESC" || id == "txtTOTAL" || id == "txtPESO" || $("#" + id).hasClass('calculo')) {
...
function GetCallPersona(obj, id) {
    ...
    if (id == "btncallage") {
        $("#txtIDAGE").val(obj.per_id);
        $("#txtCODAGE").val(obj.per_codigo);
        $("#txtNOMBRESAGE").val(obj.per_apellidos + " " + obj.per_nombres);
