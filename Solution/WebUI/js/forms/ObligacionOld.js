...
var formname = "wfObligacion.aspx";
var menuoption = "Obligacion";
...
function GetCabeceraComprobanteResult(data) {
    ...
    $("#invo").html("<i class=\"iconfa-doc\"></i> &nbsp; Retención");
    $("#invo").on("click", GeneraRetencion);
...
function Edit(row) {
    ...
    $("#txtIDCUE").val($(row.cells[0]).text()); $(row.cells[0]).text("");
    $("#txtCUENTA").val($(row.cells[1]).text()); $(row.cells[1]).text("");
    $("#txtOBSERVACION").val($(row.cells[2]).text()); $(row.cells[2]).text("");

    $("#txtCANTIDAD").val($(row.cells[3]).text()); $(row.cells[3]).text("");
    ...
    $(r.cells[3]).text(cantidad);
    $(r.cells[4]).text(valor);
    $(r.cells[5]).text(desc);
    $(r.cells[6]).text(total);
    $(r.cells[7]).text(iva);
...
function GetDcomdocObj(row) {
    ...
    if (hijocodpro.length > 0) {
        ...
        if ($.isNumeric($(row).data("codcue").toString())) {
            ...
            obj["ddco_udigitada"] = 1;
            obj["ddoc_cantidad"] = parseFloat($(row.cells[3]).text());
...
function GeneraRetencion() {
    
    
    window.location.href = "wfRetencion.aspx?tipodoc=16&origen=OBL&codigocompref=" + $("#txtcodigocomp").val() + "&codigoempresa=" + empresasigned["emp_codigo"];
