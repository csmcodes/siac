...
var formname = "wfRutaxFactura.aspx";
var menuoption = "RutaxFactura";
var asig = 0;
...
function PrintObj() {
    window.location = "wfHojaRutaPrint.aspx?codigocomp=" + $("#txtcodigocomp").val();
}
function LoadPie() {
    var obj = {};
    obj["com_empresa"] = parseInt(empresasigned["emp_codigo"]);
...
function SetFormDetalle() {
    ...
    $("#txtIDVEH").on("change", LoadFacAsig);
...
function SetAutoCompleteObj(idobj, item) {
    ...
    if (idobj == "txtIDVEH") {
        return {
            label: item.veh_nombre + "," + item.veh_disco + "," + item.veh_placa,
            value: item.veh_id,
            info: item
        }
    }
...
function ValidateForm() {
    ...
    if (htmltable.rows.length < 2) {
        retorno = false;
        mensajehtml += "Es necesario ingresar al menos una factura  a la hoja de ruta <br>";
    }
...
function GetDcomdocObj(row) {
    var obj = {};
    obj["rfac_comprobantefac"] = parseInt($(row).data("codpro").toString());
    obj["rfac_comprobantefac_key"] = parseInt($(row).data("codpro").toString());
