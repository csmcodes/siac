...
var formname = "wfDEstadoCuentanew.aspx";
var menuoption = "wfDEstadoCuentanew";
...
function GetCabeceraComprobante(per_codigo, ddo_debcre, codalmacen, fechacort,nombre) {
    if (ddo_debcre >= 0) {
        $("#txtDEBCRE").val(ddo_debcre);
    }
    if (codalmacen >= 0) {
        $("#cmbALMACEN").val(codalmacen);
    }
    if (fechacort != "") {
        $("#cmbCORTE").val(fechacort);
    }

    if (per_codigo >= 0) {
        $("#txtCODPROVEE").val(per_codigo);
        $("#cmbNOMBRE").val(nombre);
        LoadDetalle();
    }
}
...
function LoadCabeceraResult(data) {
    $("#printcartera").on("click", PrintCartera);
    $("#printestado").on("click", PrintEstado);
    if (data != "") {
        ...
        GetCabeceraComprobante($("#txtcodpersona").val(), $("#txtdebcre").val(), $("#txtcodigoalm").val(), $("#txtfechacorte").val(), $("#txtnombreplm").val()); //FUNCION DEFINIDA EN FUNCTIONS.JS
...
function PrintEstado() {
    ...
    var codalmacen = parseInt($("#cmbALMACEN").val()) || 0;
    ...
    var url = "./reports/wfReportPrint.aspx?report=ESTADOCUENTADET&empresa=" + parseInt(empresasigned["emp_codigo"]) + "&parameter1=" + $("#cmbCORTE").val() + "&parameter2=" + $("#txtCODPROVEE").val() + "&parameter3=" + $("#txtdebcre").val() + "&parameter4=" + codalmacen;
    ...
    //window.location = "wfestadocuentaprint.aspx?debcre=" + $("#txtdebcre").val() + "&codpersona=" + parseInt($("#txtCODPROVEE").val()) + "&codalmacen=" + codalmacen + "&fechacort=" + $("#cmbCORTE").val();
}

function PrintCartera() {
    //   window.location = "wfComprobantePrint.aspx?codigocomp=" + $("#txtcodigocomp").val();
    var codalmacen = parseInt($("#cmbALMACEN").val()) || 0;
    window.location = "wfcarteraprint.aspx?debcre=" + $("#txtdebcre").val() + "&codpersona=" + parseInt($("#txtCODPROVEE").val()) + "&codalmacen=" + codalmacen + "&fechacort=" + $("#cmbCORTE").val();
}
