...
namespace WebUI
{
    public partial class wfReporteComprobantesTransporte : System.Web.UI.Page
    {
        ...
        public static string GetFiltros(object objeto)
        {
            ...
            tdatos.rows[4].cells[0].valor = "Ruta:";
            tdatos.rows[4].cells[1].valor = new Select { id = "cmbRUTA", diccionario = Dictionaries.Empty(), clase = Css.large, withempty = true }.ToString();
            tdatos.rows[5].cells[0].valor = "Estado:";
            tdatos.rows[5].cells[1].valor = new Select { id = "cmbESTADO", valor ="1,2", clase = Css.large, diccionario = Dictionaries.GetEstadoComprobante(), multiselect = true, data = "data-placeholder=\"Seleccione Estado...\"", withempty = true }.ToString();
            tdatos.rows[6].cells[0].valor = "";
            tdatos.rows[6].cells[1].valor = new Boton { click = "LoadReporte();return false;", valor = "Reporte" }.ToString() ;
        ...
        [WebMethod]
        public static string GetRutas(object objeto)
        {
            ...
            if (!string.IsNullOrEmpty((string)almacen))
            {
                ...
                return new Select { id = id.ToString(), diccionario = Dictionaries.GetRutaByAlmacen(int.Parse(empresa.ToString()),int.Parse(almacen.ToString())), clase = Css.large, withempty = true }.ToString();
