...
namespace WebUI
{
    public partial class wfReporteElectronicos : System.Web.UI.Page
    {
        ...
        public static string GetFiltros(object objeto)
        {
            ...
            tdatos.rows[4].cells[0].valor = "Tipo Comprobante:";
            tdatos.rows[4].cells[1].valor = new Select { id = "cmbTIPO", diccionario = Dictionaries.GetElectronicos(), clase = Css.large, withempty = true }.ToString();
            tdatos.rows[5].cells[0].valor = "Estado Electróinco:";
            tdatos.rows[5].cells[1].valor = new Select { id = "cmbESTADO", diccionario = Dictionaries.GetEstadosElectronico(), clase = Css.large, withempty = true }.ToString();
            tdatos.rows[6].cells[0].valor = "";
            tdatos.rows[6].cells[1].valor = new Boton { click = "LoadReporte();return false;", valor = "Ver Reporte" }.ToString() + " " + new Boton { click = "Verificar();return false;", valor = "Verificar Comprobantes" }.ToString();
        ...
        [WebMethod]
        public static string VerificarComprobantes(object objeto)
        {
            ...
            DateTime? dtdesde = Conversiones.ObjectToDateTimeNull(desde);
            DateTime? dthasta= Conversiones.ObjectToDateTimeNull(hasta);


            Packages.General.VerificarComprobantesElectronicos(dtdesde.Value, dthasta.Value, com.com_almacen, com.com_pventa, com.com_empresa, com.com_tipodoc, com.com_estadoelec);

            return "Verificando comprobantes, espere por favor...";
