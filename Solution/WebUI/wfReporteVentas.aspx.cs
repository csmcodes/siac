...
namespace WebUI
{
    public partial class wfReporteVentas : System.Web.UI.Page
    {
        ...
        public static string GetFiltros(object objeto)
        {
            ...
            tdatos.rows[4].cells[0].valor = "Politica";
            tdatos.rows[4].cells[1].valor = new Select { id = "cmbPOLITICA", clase = Css.large, diccionario = Dictionaries.GetPolitica(4), multiselect = true, data = "data-placeholder=\"Seleccione Politica...\"", withempty = true }.ToString();

            tdatos.rows[5].cells[0].valor = ":";
            //tdatos.rows[5].cells[1].valor = new Boton { click = "LoadRep();return false;", valor = "Ventas" }.ToString() + " " + new Boton { click = "LoadReporte();return false;", valor = "Ventas Detallado" }.ToString() + " " + new Boton { click = "LoadReporteSocio();return false;", valor = "Ventas x Socio" }.ToString();
            tdatos.rows[5].cells[1].valor = new Boton { click = "LoadRep();return false;", valor = "Ventas" }.ToString() + " " + new Boton { click = "LoadReporte();return false;", valor = "Ventas Detallado" }.ToString();
