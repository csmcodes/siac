...
namespace WebUI
{
    public partial class wfMensaje : System.Web.UI.Page
    {
        ...
        public static string ShowData(Mensaje obj)
        {
            ...
            return new HtmlObjects.ListItem { titulo = new string[] { obj.msj_id }, descripcion = new string[] { }, logico = new LogicItem[] { new LogicItem("Activos", obj.msj_estado) } }.ToString();
        }
        ...
        public static string ShowObject(Mensaje obj)
        {
           ...
           // html.AppendLine(new Select { id = "cmbDESTINO", etiqueta = "Para", placeholder = "Para", diccionario = Dictionaries.Empty(), multiselect = true, obligatorio = true }.ToString());
            html.AppendLine(new Select { id = "cmbDESTINO", etiqueta = "Para", diccionario = Dictionaries.Empty(), multiselect = true, obligatorio = true }.ToString());
            html.AppendLine(new Input { id = "txtASUNTO", etiqueta = "Asunto", placeholder = "Asunto", valor = obj.msj_asunto, clase = Css.large }.ToString());
            html.AppendLine(new Textarea { id = "txtMENSAJE", etiqueta = "Mensaje", placeholder = "Mensaje", valor = obj.msj_mensaje, cols = 30, rows = 15, clase = Css.xlarge, obligatorio = true }.ToString());
    //      html.AppendLine(new Input { id = "cmbFECHACREACION",  datepicker = true, datetimevalor = DateTime.Now, clase = Css.large, visible = false}.ToString());
            ...
            html.AppendLine(new Input { id = "txtUSUARIOENVIA", valor = obj.msj_usuarioenvia, visible = false, obligatorio = true }.ToString());
        ...
        public static string GetDestino(object objeto)
        {
           ...
           // html.AppendLine(new Select { id = "cmbDESTINO", diccionario = Dictionaries.GetUsuarioxEmpresa(Convert.ToInt32(empresa)), multiselect = true }.ToString());
           ...
           // html.AppendLine(new Select { id = "cmbDESTINO", etiqueta = null, valor = "", diccionario = Dictionaries.GetUsuarioxEmpresa(Convert.ToInt32(empresa)), multiselect = true, obligatorio = true }.ToString());
            return html.ToString();
        ...
        public static void SetWhereClause(Mensaje obj)
        {
            ...
            if (obj.msj_codigo > 0)
            {
                parametros.where += ((parametros.where != "") ? " and " : "") + " msj_codigo = {" + contador + "} ";
                valores.Add(obj.msj_codigo);
                contador++;
            }
           
            if (!string.IsNullOrEmpty(obj.msj_id))
            {
                parametros.where += ((parametros.where != "") ? " and " : "") + " msj_id like {" + contador + "} ";
                valores.Add("%" + obj.msj_id + "%");
                contador++;
            }
        ...
        public static string SaveObject(object objeto)
        {
            ...
            if (obj.msj_codigo_key > 0)
            {
                ...
                List<Mensajedestino> lst = MensajedestinoBLL.GetAll("msj_codigo =" + obj.msj_codigo, "");
