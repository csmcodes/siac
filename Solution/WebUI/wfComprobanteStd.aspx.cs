...
namespace WebUI
{
    public partial class wfComprobanteStd : System.Web.UI.Page
    {
        ...
        public static string GetProduct(object objeto)
        {
            ...
            if (objeto != null)
            {
                ...
                if (lst.Count > 0)
                {
                    ...
                    product.tproducto.calculaprecio = CalculoprecioBLL.GetAll(new WhereParams("cpr_empresa ={0} and cpr_tproducto={1} and cpr_listaprecio = {2} and cpr_almacen={3} ", product.pro_empresa, product.pro_tproducto, int.Parse(lista.ToString()), int.Parse(almacen.ToString())), "");
                }
            }
        ...
        public static string GetProductPrice(object objeto)
        {
            ...
            if (objeto != null)
            {
                ...
                List<Dlistaprecio> lst = DlistaprecioBLL.GetAll(new WhereParams("dlpr_empresa = {0} and dlpr_producto = {1} and dlpr_listaprecio = {2} and dlpr_umedida= {3} and dlpr_almacen={5} and ((dlpr_fecha_ini >= {4} and dlpr_fecha_fin IS NULL) or ({4} between dlpr_fecha_ini and dlpr_fecha_fin))", 1, int.Parse(producto.ToString()), int.Parse(lista.ToString()), int.Parse(unidad.ToString()), DateTime.Now, int.Parse(almacen.ToString())), "");
                if (lst.Count > 0)
