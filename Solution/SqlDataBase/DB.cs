...
namespace SqlDataBase
{
    public class DB
    {
        ...
        {
            SqlCommand cmd = CreateCommand(cmdText, CommandType.Text, transaction);
        ...
        public static int DeleteAll(TransactionManager transaction, string WhereClause, string tablename)
        {
            ...
            SqlCommand cmd = CreateCommand(sql, CommandType.Text, transaction);
            return ExecuteNonQuery(cmd);
        }
        ...
        {
            SqlCommand cmd = CreateCommand(cmdText, CommandType.StoredProcedure, transaction);
        ...
        private static SqlCommand CreateCommand(string cmdText, CommandType cmdType, TransactionManager transaction)
        {
            SqlCommand cmd = new SqlCommand(cmdText, transaction.connection, transaction.transaction);
        ...
        public static SqlParameter AddParameter(string name, SqlDbType tipo, object dato)
        {
            SqlParameter parametro = new SqlParameter(name, tipo);
            if (dato != null)
        ...
        private static SqlCommand AddParameters(SqlCommand cmd, PropertyInfo[] properties, SentenceType tipo, object context)
        {
        ...
        private static SqlCommand AddParameters(SqlCommand cmd, WhereParams parametros)
        {
            int contador = 0;
        ...
        private static int ExecuteNonQuery(SqlCommand dCmd)
        {
        ...
        private static object ExecuteEscalar(SqlCommand dCmd)
        {
            /*object res = null;
            try
            {
        ...
        private static IDataReader ExecuteReader(SqlCommand dCmd)
        {
        ...
        private static DataSet ExecuteDataSet(SqlCommand dCmd)
        {
