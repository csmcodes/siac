...
namespace BusinessObjects
{
    public class vGuias
    {
        ...
        public vGuias(IDataReader reader)
        {
        ...
        public string GetSQL()
        {
            string sql = "SELECT " +
                            ...
                            " tot_total" +
                        " FROM comprobante " +
                            " LEFT JOIN planillacli ON com_empresa= plc_empresa and com_codigo = plc_comprobante " +
        ...
        public string GetSQLPlanilla()
        {
            string sql = "SELECT " +
                           ...
                           " tot_subtot_0," +
                            ...
                            " tot_desc1_0," +
                            " tot_total" +
                         " FROM planillacomprobante " +
                           " INNER JOIN planillacli ON pco_empresa=plc_empresa AND pco_comprobante_pla = plc_comprobante_pla " +
                           " INNER JOIN comprobante ON plc_empresa= com_empresa AND plc_comprobante = com_codigo " +
                           " LEFT JOIN ccomenv ON com_empresa = cenv_empresa AND com_codigo = cenv_comprobante " +
                           " LEFT JOIN persona ON cenv_empresa = per_empresa AND cenv_socio = per_codigo " +
                           " INNER JOIN total ON com_empresa = tot_empresa AND com_codigo = tot_comprobante";
        ...
        public List<vGuias> GetStruc()
        {
            return new List<vGuias>();
        }
